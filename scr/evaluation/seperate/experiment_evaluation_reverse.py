"""
Modul: Experiment Evaluator (Phase 4 - REVERSE OOS)
Führt Out-of-Sample Tests (Test auf den ersten 30%, Train auf den letzten 70%) durch.
Loggt alle Ergebnisse zentral, um verschiedene Experimente vergleichbar zu machen.
"""
...
class ExperimentEvaluator:
    ...
    def run(self):
        ...
        # Split liegt jetzt bei 30% der Zeitleiste
    ...
    def _create_and_save_plot(self, equity_curve, split_idx, p_train, w_train, dd_train, sh_train, so_train,
        ...
        # ---------------------------------------------------------
        # FARBEN UND LABELS FÜR REVERSE-PLOT GETAUSCHT
        # ---------------------------------------------------------
        # Der linke Teil (bis split_idx) ist jetzt OOS (Grün)
        plt.plot(range(split_idx), equity_curve[:split_idx], color='green', linewidth=1.5, label='Out-of-Sample (Test, 2020-2022)')
        
        if split_idx > 0 and split_idx < len(equity_curve):
            # Der rechte Teil (ab split_idx) ist jetzt In-Sample/Train (Blau)
            plt.plot(range(split_idx-1, len(equity_curve)), equity_curve[split_idx-1:], color='blue', linewidth=1.5, label='In-Sample (Train, 2022-2026)')
...
if __name__ == "__main__":
    ...
    EXPERIMENT = "Reverse_OOS_Ohne_Friday_Exit_500_Trials"
    
    # 2. Die Parameter-Datei, die vom angepassten master_loop_reverse erstellt wurde
    PARAMETER_DATEI = "params_WITHOUT_friday_exit.csv"
