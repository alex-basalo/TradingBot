...
class EnsembleEvaluator:
    ...
    def _extract_trades(self, df: pd.DataFrame, params: dict, use_friday_exit: bool) -> list:
        # Indikatoren anwenden (inkl. Kat C)
        ...
        # Kat C (Trend) bei den Signalen berücksichtigen
    ...
    def run(self):
        ...
        print(f"\nStarte ENSEMBLE Auswertung (Hybrid 70% Train / 30% Test): '{self.experiment_name}'")
        print("=" * 75)
        ...
        # =========================================================
        # STANDARD SPLIT LOGIK (Erste 70% = Train, Letzte 30% = Test)
        # =========================================================
        all_trades.sort(key=lambda x: x['time'])
        first_date, last_date = all_trades[0]['time'], all_trades[-1]['time']
        
        # Split bei 70% der Zeitleiste
        split_date = first_date + (last_date - first_date) * 0.7

        # Die ersten 70% der Zeitachse sind das Training (In-Sample)
        train_trades = [t for t in all_trades if t['time'] < split_date]
        
        # Die letzten 30% der Zeitachse sind der Test (OOS)
        oos_trades = [t for t in all_trades if t['time'] >= split_date]
        
        # Index für den Graphen, wo Train endet und OOS beginnt
        ...
        # Equity Curve chronologisch berechnen
    ...
    def _create_and_save_plot(self, equity_curve, split_idx, p_train, w_train, dd_train, sh_train, so_train,
        ...
        # ---------------------------------------------------------
        # FARBEN UND LABELS (Blau = Train, Grün = Test)
        # ---------------------------------------------------------
        # Der LINKE Teil (0 bis split_idx) sind die ersten 70% = In-Sample/Train (BLAU)
        plt.plot(range(split_idx), equity_curve[:split_idx], color='blue', linewidth=1.5, label='In-Sample (Train, Erste 70%)')
        
        if split_idx > 0 and split_idx < len(equity_curve):
            # Der RECHTE Teil (ab split_idx) sind die letzten 30% = Out-of-Sample/Test (GRÜN)
            plt.plot(range(split_idx-1, len(equity_curve)), equity_curve[split_idx-1:], color='green', linewidth=1.5, label='Out-of-Sample (Test, Letzte 30%)')
            plt.axvline(x=split_idx, color='red', linestyle='--', linewidth=2, label='Start OOS (Test)')
        # ---------------------------------------------------------
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(self.plots_dir, f"{self.experiment_name}_Equity_500.png"), dpi=300)
        plt.close()
...
if __name__ == "__main__":
    ...
    EXPERIMENT = "Ensemble_Hybrid_Modell_Kat_C"
    
    # Trage hier die beiden CSV-Dateien ein, die das Skript vergleichen soll
    DATEI_MIT_EXIT = "params_WITH_friday_exit_kat_c_(1000).csv"
    DATEI_OHNE_EXIT = "params_WITHOUT_friday_exit_kat_c_(1000).csv"
